...
# ---------------------------------------------------------------------------
# Kiem tra "da cai" bang cach quet ten file trong thu muc mods/resourcepacks/
# shaderpacks cua Instance, roi so sanh THEO TEN voi mod/rsp/shader dang xem
# trong mod_mc / mod_detail_window. Cach nay bat duoc ca nhung mod nguoi dung
# tu tay bo vao thu muc (khong cai qua launcher nen khong co trong index json).
#
# De toi uu (khong doc lai o dia cho tung dong trong danh sach), ket qua quet
# thu muc duoc cache theo (ten_instance, loai) kem mtime cua thu muc: chi khi
# mtime doi khac (co file them/bot) moi goi lai os.listdir; neu khong, tra ve
# thang tu cache trong bo nho.
# ---------------------------------------------------------------------------
_cache_quet_thu_muc = {}   # (ten_instance, loai) -> (mtime, {ten_chuan: ten_file_goc})
...
def _quet_thu_muc_map(ten_instance, loai):
    """Quet thu muc mods/resourcepacks/shaderpacks cua 1 Instance, tra ve dict
    {ten_da_chuan_hoa: ten_file_goc}. Ket qua duoc cache theo (ten_instance, loai)
    kem mtime cua thu muc de toi uu: chi doc lai o dia khi thu muc thuc su thay
    doi (them/bot file), con lai dung ngay ket qua da cache trong bo nho."""
...
def kiem_tra_ten_da_cai(ten_instance, loai, ten_hien_thi):
    """So sanh ten mod/rsp/shader (vd tren Modrinth/CurseForge) voi cac file
    dang co trong thu muc tuong ung cua Instance - chi so sanh theo TEN, khong
    quan tam nguon/id/phien ban."""
...
def tim_ten_file_da_cai(ten_instance, loai, ten_hien_thi):
    """Giong kiem_tra_ten_da_cai nhung tra ve ten file GOC (vd 'sodium-1.20.jar')
    thay vi True/False. Dung khi can biet chinh xac file nao dang nam trong thu
    muc de xoa di truoc khi cap nhat len phien ban moi (truong hop mod duoc cai
    thu cong nen khong co trong index, khong biet ten file chinh xac)."""
    ten_chuan = _chuan_hoa_ten(ten_hien_thi)
    if not ten_chuan:
        return None
...
def xoa_file_theo_ten(ten_instance, loai, filename):
    """Xoa 1 file (mod/resourcepack/shaderpack) da cai theo ten file, dung khi
    cap nhat len phien ban moi de don dep file cu, tranh 2 ban (cu + moi) cung
    ton tai trong 1 thu muc gay xung dot."""
