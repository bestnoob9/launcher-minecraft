...
from components.instance_common import luu_anh_bia
...
class InstanceFrame(tk.Frame):
    ...
    def xoa_instance_theo_ten(self, ten=None):
        ...
        if ten_instance == "Latest Version":
            ...
            return
        app = self.app
        proc = getattr(app, "_game_process", None) if app is not None else None
        if proc is not None and proc.poll() is None:
            ten_dang_chay = getattr(app, "_ten_instance_dang_chay", None)
            if ten_dang_chay == ten_instance:
                thong_bao = "Vui lòng tắt game trước khi xóa phiên bản đang chạy!"
                if self.modal is not None:
                    self.modal.alert("Chú ý", thong_bao)
                else:
                    messagebox.showwarning("Chú ý", thong_bao)
        ...
        def _thuc_hien_xoa():
            ...
            duong_dan_goc = os.path.join(self.thu_muc_instances, ten_folder)
            duong_dan_rac = None
            if os.path.exists(duong_dan_goc):
                dau_thoi_gian = time.strftime("%Y%m%d_%H%M%S")
                duong_dan_rac = os.path.join(
                    self.thu_muc_instances, f"_trash_{ten_folder}_{dau_thoi_gian}")
                try:
                    os.rename(duong_dan_goc, duong_dan_rac)
                except Exception as e:
                    thong_bao_loi = (
                        f"Không thể xóa '{ten_instance}':\n{e}\n\n"
                        "Hãy tắt game hoặc đóng thư mục instance này (Explorer, "
                        "trình soạn thảo,...) rồi thử xóa lại."
                    )
                    if self.modal is not None:
                        self.modal.alert("Lỗi", thong_bao_loi)
                    else:
            ...
            if app is not None:
                try:
                    if getattr(app, "_home_mode", None) == "detail":
                        detail = getattr(app, "_home_detail_frame", None)
                        if detail is not None and getattr(detail, "ten_instance", None) == ten_instance:
                            app.back_to_library()
                except Exception:
                    pass
            self.on_change_callback()
            if self.modal is not None:
                self.modal.toast(f"Đã xóa phiên bản '{ten_instance}'.", loai="ok")
            if duong_dan_rac is not None:
                def _don_rac_nen(path=duong_dan_rac):
                    try:
                    ...
                    try:
                        if os.name == "nt":
                            ...
                            if ket_qua.returncode == 0 or not os.path.exists(path):
                                return
                        import shutil
                        shutil.rmtree(path, ignore_errors=True)
                    except Exception as e:
                        print(f"[xoa_instance] Không thể dọn hẳn thư mục rác '{path}': {e}")
                threading.Thread(target=_don_rac_nen, daemon=True).start()
        if self.modal is not None:
            self.modal.confirm(
                title="Xóa phiên bản",
    ...
    def build_create_panel(self, parent, close):
        ...
        picker_anh_bia = CoverPickerPanel(content, duong_dan_ban_dau=None)
        picker_anh_bia.pack(fill="x", pady=(10, 0))
        def xu_ly_tao():
            ...
            duong_dan_anh_bia = picker_anh_bia.get_duong_dan()
            if duong_dan_anh_bia:
                luu_anh_bia(ten_nhap, duong_dan_anh_bia)
            ds_moi = list(config.current_config["danh_sach_instances"].keys())
            self.selector.set_items(ds_moi)
