...
_TEN_LOADER_BIET = {"forge", "fabric", "quilt", "neoforge", "vanilla",
                     "liteloader", "rift"}
_LOAI_KHONG_CAN_LOADER = {"resourcepacks", "shaderpacks"}
def _ver_sort_key(s):
...
class ModDetailWindow(tk.Frame):
    ...
    def _build_ui(self):
        ...
        self._bind_wheel(self.txt_intro, self.txt_intro)
        ...
        self._bind_wheel(self.txt_log, self.txt_log)
        ...
        self._bind_wheel(self._gal_canvas, self._gal_canvas)
        self._bind_wheel(self._gal_inner, self._gal_canvas)
        ...
        self._ver_col_w = [34, 0, 150, 100, 70, 70, 80, 40]  
        ...
        for wdg in (self._ver_canvas, self._ver_table_inner):
            self._bind_wheel(wdg, self._ver_canvas)
    ...
    def _load_gallery_thumbnails(self):
        ...
        def _t():
            ...
            so_worker = min(perf.get_perf().get("tai_icon", 3), len(urls))
            with concurrent.futures.ThreadPoolExecutor(
                    max_workers=so_worker) as ex:
    ...
    def _render_gallery(self, results):
        ...
        for i, (url, photo) in enumerate(ok_results):
            ...
            self._bind_wheel_recursive(cell, self._gal_canvas)
    ...
    def _on_ver_mousewheel(self, e):
        ...
        self._ver_canvas.yview_scroll(delta * 3, "units")
    def _wheel_scroll_units(self, e):
    ...
    def _bind_wheel(self, widget, scrollable):
        def _wheel(e):
            try:
                scrollable.yview_scroll(self._wheel_scroll_units(e), "units")
            except tk.TclError:
                pass
            return "break"
        widget.bind("<MouseWheel>", _wheel, add=False)
        widget.bind("<Button-4>", _wheel, add=False)
        widget.bind("<Button-5>", _wheel, add=False)
    def _bind_wheel_recursive(self, widget, scrollable):
        if isinstance(widget, (ttk.Combobox, ttk.Entry, tk.Entry)):
            return
        self._bind_wheel(widget, scrollable)
        for child in widget.winfo_children():
            self._bind_wheel_recursive(child, scrollable)
    ...
    def _apply_instance_lock_to_filters(self, gv_opts=None, loader_opts=None):
        ...
        if ten_inst and mcv:
            ...
            if self._loai in _LOAI_KHONG_CAN_LOADER:
                ...
                if not mc_ok:
                    self._ver_bao_loi = (
                        f"Không tìm thấy phiên bản nào hỗ trợ Minecraft {mcv} "
                        f"(yêu cầu của Instance '{ten_inst}').")
            elif self._loai == "mods" and (not loader_l or loader_l == "vanilla"):
    ...
    def _build_ver_row(self, row_i, vidx):
        ...
        self._bind_wheel_recursive(row, self._ver_canvas)
    ...
    def _lay_khoa_so_sanh_phien_ban(self, version_data):
        vid = str(version_data.get("id", "") or "")
        vnum = str(version_data.get("version_number")
                    or version_data.get("displayName") or "").strip()
        if self._source == "modrinth":
            ngay = version_data.get("date_published", "") or ""
        else:
            ngay = version_data.get("fileDate", "") or ""
        return vid, vnum, ngay
    def _cap_nhat_nhan_nut_cai_dat(self):
        ...
        if not self._installed_info:
            ...
            vid, vnum, ngay = self._lay_khoa_so_sanh_phien_ban(self._versions[idx])
            old_vid  = str(self._installed_info.get("version_id") or "")
            old_vnum = str(self._installed_info.get("version_number") or "").strip()
            old_ngay = self._installed_info.get("ngay", "") or ""
            def _nhan_theo_ngay(mac_dinh):
                if ngay and old_ngay:
                    if ngay < old_ngay:
                        return "⬇  Hạ phiên bản"
                    if ngay > old_ngay:
                        return "⬆  Cập nhật"
            ...
            if vid and old_vid:
                if vid == old_vid:
                    ...
                    self._nhan_nut_hien_tai = _nhan_theo_ngay("⬆  Cập nhật")
            elif vnum and old_vnum:
                if vnum.lower() == old_vnum.lower():
                    self._nhan_nut_hien_tai = "🔁  Cài lại"
                else:
                    self._nhan_nut_hien_tai = _nhan_theo_ngay("⬆  Cập nhật")
            elif ngay and old_ngay:
                self._nhan_nut_hien_tai = _nhan_theo_ngay("🔁  Cài lại")
            else:
                self._nhan_nut_hien_tai = "🔁  Cài lại"
        if not self._dang_cai:
            try:
