...
class SettingFrame(tk.Frame):
    ...
    def _build_widgets(self, p):
        ...
        tk.Label(p, text="Tối ưu hiệu năng theo máy:", font=("Arial", 10, "bold")).pack(anchor="w", padx=20, pady=(15, 2))
        frame_perf = tk.Frame(p)
        frame_perf.pack(fill="x", padx=20)
        tk.Label(frame_perf, text="Chọn nhanh:", font=("Arial", 9)).pack(side=tk.LEFT)
        self.cbo_perf_profile = ttk.Combobox(
            frame_perf,
            values=list(perf.TEN_PROFILE_HIEN_THI.values()),
            width=20, state="readonly"
        )
        self.cbo_perf_profile.pack(side=tk.LEFT, padx=10)
        _perf_luu = config.current_config.get("perf_profile", "auto")
        self.cbo_perf_profile.set(perf.TEN_PROFILE_HIEN_THI.get(_perf_luu, "Tự động"))
        _profile_dang_ap_dung = perf.profile_hien_tai()
        tk.Label(
            frame_perf,
            text=f"(máy này: {perf.TEN_PROFILE_HIEN_THI.get(_profile_dang_ap_dung, _profile_dang_ap_dung)})",
    ...
    def _setup_dirty_tracking(self):
        ...
        for cbo in (self.cbo_size_preset, self.cbo_res_preset,
                    self.cbo_jvm_mode, self.cbo_jvm_presets, self.cbo_perf_profile):
            try:
    ...
    def _thong_bao_toast(self, message):
        if self.modal is not None:
    ...
    def luu_cau_hinh(self):
        ...
        ten_perf_hien_thi = self.cbo_perf_profile.get()
        config.current_config["perf_profile"] = perf._HIEN_THI_TOI_PROFILE.get(
            ten_perf_hien_thi, "auto")
...
class SettingWindow(tk.Toplevel):
    ...
    def luu_cau_hinh(self):
        ...
        self.ent_jvm_custom.configure(state="readonly" if jvm_ui_mode == "Sử dụng gói tối ưu sẵn" else "normal")
        config.luu_toan_bo_cau_hinh()
        self._thong_bao_toast("Đã lưu toàn bộ cấu hình hệ thống!")
        self.on_save_callback()
        self.destroy()