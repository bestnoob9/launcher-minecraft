...
MICROSOFT_CLIENT_ID = ""
MICROSOFT_LOGIN_PORT = 17389
MICROSOFT_REDIRECT_URI = f"http://127.0.0.1:{MICROSOFT_LOGIN_PORT}"
...
_ASSETS_DIR = os.path.join(_thu_muc_bundle(), "assets")
...
def dong_bo_username_json(thu_muc_game: str = "", danh_sach_acc: list = None) -> dict:
    ...
    ten_offline = []
    for item in danh_sach_acc:
        if isinstance(item, dict):
            if item.get("type", "offline") == "offline":
                ten_offline.append(item.get("name", ""))
        else:
            ten_offline.append(item)
    du_lieu = doc_username_json(thu_muc_game)
    thay_doi = False
    for ten in ten_offline:
        ten = (ten or "").strip()
    ...
    if thay_doi:
        ghi_username_json(du_lieu, thu_muc_game)
    return du_lieu
def _chuan_hoa_1_acc(item):
    if isinstance(item, str):
        ten = item.strip()
        return {"type": "offline", "name": ten} if ten else None
    if isinstance(item, dict):
        ten = (item.get("name") or "").strip()
        if not ten:
            return None
        loai = item.get("type") if item.get("type") in ("offline", "microsoft") else "offline"
        d = {"type": loai, "name": ten}
        if loai == "microsoft":
            d["uuid"] = item.get("uuid", "")
            d["refresh_token"] = item.get("refresh_token", "")
        return d
    return None
def chuan_hoa_danh_sach_acc(danh_sach_acc) -> list:
    ket_qua = []
    ten_da_co = set()
    for item in (danh_sach_acc or []):
        d = _chuan_hoa_1_acc(item)
        if not d or d["name"] in ten_da_co:
            continue
        ten_da_co.add(d["name"])
        ket_qua.append(d)
    return ket_qua
def ten_danh_sach_acc(danh_sach_acc) -> list:
    return [d.get("name", "") for d in (danh_sach_acc or [])]
def tim_tai_khoan(danh_sach_acc, ten):
    for d in (danh_sach_acc or []):
        if d.get("name") == ten:
            return d
    return None
...
def tai_toan_bo_cau_hinh():
    ...
    data["danh_sach_acc"] = chuan_hoa_danh_sach_acc(data.get("danh_sach_acc", []))
