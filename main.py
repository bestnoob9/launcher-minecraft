...
class MinecraftLauncherApp:
    ...
    def __init__(self, root):
        ...
        self._ten_instance_dang_chay = None   
    ...
    def _build_home_view(self, parent) -> tk.Frame:
        ...
        from components.home_library import HomeLibraryFrame
        self._home_mode = "library"
        self._home_detail_frame = None
        self._home_library = HomeLibraryFrame(frame, self)
        self._home_library.pack(fill="both", expand=True)
        return frame
    @property
    def instance_detail(self):
        if getattr(self, "_home_mode", None) != "detail":
            return None
        return getattr(self, "_home_detail_frame", None)
    def open_instance_detail(self, ten: str):
        if not ten:
            return
        if self.instance_frame.get_current_instance() != ten:
            self.instance_frame.selector.set(ten)
            self.instance_frame._khi_chon_selector(ten)
        home_frame = self._view_frames["home"]
        self._home_library.pack_forget()
        from components.instance_detail import InstanceDetailFrame
        if self._home_detail_frame is not None:
            self._home_detail_frame.destroy()
        self._home_detail_frame = InstanceDetailFrame(home_frame, self, ten)
        self._home_detail_frame.pack(fill="both", expand=True)
        self._home_mode = "detail"
    def back_to_library(self):
        ...
        self._home_mode = "library"
        self._home_library.pack(fill="both", expand=True)
        self._home_library.refresh()
    def _cap_nhat_home_view(self):
        if not hasattr(self, "_home_mode"):
            return
        try:
            if self._home_mode == "detail" and self._home_detail_frame is not None:
                cur = self.instance_frame.get_current_instance()
                self._home_detail_frame.set_instance(cur)
            elif hasattr(self, "_home_library"):
                self._home_library.refresh()
        except Exception:
            pass
    def _build_modpack_view(self, parent) -> tk.Frame:
        from components.mod_mc import ModMcFrame
        frame = ModMcFrame(parent, callback_lam_moi=self._lam_moi_instance_frame, modal=self.modal)
        frame.app = self
        return frame
    ...
    def bat_dau_chay_game(self):
        _ten_tai_khoan = self.account_frame.get_current_account()
        if not _ten_tai_khoan:
            self.modal.alert("Chú ý", "Vui lòng chọn hoặc thêm tài khoản trước khi chơi!")
            return
        tai_khoan = config.tim_tai_khoan(
            config.current_config.get("danh_sach_acc", []), _ten_tai_khoan
        ) or {"type": "offline", "name": _ten_tai_khoan}
        self._dang_tai = True
        ...
        def luong_khoi_dong():
            try:
                ...
                self._ten_instance_dang_chay = ten_instance
                if self._huy_tai:
                    self._dang_tai = False
                    self._huy_tai = False
